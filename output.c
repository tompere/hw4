...
CONTINUE:
	...
	/* create symbol table based on constants */
		/* Allocate memory and create the SOB string: "hi
" */
	PUSH(104);
	PUSH(105);
	PUSH(10);
	...
	MOV(ADDR(16),R0);
	DROP(IMM(4));
	/* Allocate memory and create the SOB integer: 7 */
	PUSH(IMM(7));
	CALL(MAKE_SOB_INTEGER);
	MOV(ADDR(17),R0);
	DROP(IMM(1));
	/* Allocate memory and create the SOB char: g */
	PUSH(IMM(103));
	CALL(MAKE_SOB_CHAR);
	MOV(ADDR(18),R0);
	DROP(IMM(1));
